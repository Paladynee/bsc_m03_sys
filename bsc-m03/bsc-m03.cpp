...
#include "libbscsais/libbscsais.h"
#include "libbscsais/libbscsais16.h"
...
template <class symbol_t> static int32_t compress_memory_block(uint8_t * buffer, int32_t block_size)
{
    ...
    if (int32_t * libbscsais_temp = (int32_t *)malloc(block_symbols * sizeof(int32_t)))
    {
        int32_t result = symbol_size == 1
            ? libbscsais_bwt_aux(buffer, buffer, libbscsais_temp, block_symbols, 0, root_frequencies, r, indexes)
            : libbscsais16_bwt_aux((uint16_t *)buffer, (uint16_t *)buffer, libbscsais_temp, block_symbols, 0, root_frequencies, r, indexes);

        free(libbscsais_temp);
        ...
        if (result == 0)
        {
            if (symbol_t * L = (symbol_t *)malloc(((size_t)block_symbols + 1) * sizeof(symbol_t)))
            {
                ...
                if (compressed_size >= block_size)
                {
                    ...
                    if (int32_t * libbscsais_temp = (int32_t *)malloc(((size_t)block_symbols + 1) * sizeof(int32_t)))
                    {
                        {
                            int32_t primary_index = indexes[0];
                        ...
                        result = symbol_size == 1
                            ? libbscsais_unbwt_aux((uint8_t *)buffer, (uint8_t *)buffer, libbscsais_temp, block_symbols, root_frequencies, r, indexes)
                            : libbscsais16_unbwt_aux((uint16_t *)buffer, (uint16_t *)buffer, libbscsais_temp, block_symbols, root_frequencies, r, indexes);
                        ...
                        {
                            fprintf(stderr, "\nError: libbscsais_unbwt failed, please contact the author!\n");
                        }
        ...
        {
            fprintf(stderr, "\nError: libbscsais_bwt failed, please contact the author!\n");
        }
    }
    else
    {
...
static int32_t decompress_memory_block(uint8_t * buffer, int32_t compressed_size, int32_t block_size)
{
    ...
    if (result == 0)
    {
        ...
        {
            result = symbol_size == 1
                ? libbscsais_unbwt_aux(buffer, buffer, libbscsais_temp, block_symbols, root_frequencies, r, indexes)
                : libbscsais16_unbwt_aux((uint16_t *)buffer, (uint16_t *)buffer, libbscsais_temp, block_symbols, root_frequencies, r, indexes);
